...
\begin{abstract}
  ...
  {\em i.e. black holes}, of less than about one quarter,
  $-0.53<\eta_N<0.26$
  from M\,32 from using the best determined distances to M\,31 and
  M\,32.  If M\,31 and M\,32 have the maximal relative separation
  allowed within the uncertainties of \citet{2006AJ....131.1405K}, the
  limit is much weaker with $\eta_N<0.99$.  The constraints from
  M\,87 are substantially weaker but could improve dramatically with
  better astrometry.
\end{abstract}
...
If one directly used Eq.~(\ref{eq:4}) to calculate the restroring
force toward the centre, one would find that it is constant with
...
with 0.5-arcsecond astromety and $|\Delta|<3\times 10^{-3}$ with
...
$-0.53<\eta_N<0.26$, competitive with the results from pulsar timing
\citep{Stairs:2005}.

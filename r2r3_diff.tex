...
%DIF ADD weighbh.tex      Fri Aug  5 10:25:51 2016
\usepackage{graphicx}
\usepackage{url}
\usepackage{tikz}
...
, the limit is much weaker with $\eta_N<0.99$.  The constraintsfrom
  ...
  ellipticals of }\DIFaddend M\,\DIFdelbegin \DIFdel{87 are substantially weaker but could }\DIFdelend \DIFaddbegin \DIFadd{31, substantially better constraints could be obtained. In all
  case the constraints could }\DIFaddend improve dramatically with better astrometry.
\end{abstract}
...
by violation of the equivalence principle, Eq.~\ref{eq:3}, then we
...
density, Eq.~\ref{eq:4}.  On the other hand, we will assume that the
...
If one directly used Eq.~(\ref{eq:4}) to calculate the \DIFdelbegin \DIFdel{restroring
}\DIFdelend \DIFaddbegin \DIFadd{restoring
}\DIFaddend force toward the centre, one would find that it is constant with
...
it.  We will compare this estimate with the detailed measurements of
the potential near the core of the elliptical galaxy, M\,87, and find
good agreement.
...
\DIFadd{If we examine Eq.~\ref{eq:5}, we see that the spring constant of the
...
Eq.~(\ref{eq:17}) and a favourable geometry of $i\approx 0^\circ$.
...
several reasons.\footnote{Note also the environmental dependence of
  $\Delta$ is often such that $\Delta$ on the left hand side of Eq.~(\ref{eq:3})

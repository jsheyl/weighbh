...
%DIF DEL weighbh_r2.tex   Thu Aug  4 11:07:27 2016
%DIF ADD weighbh.tex      Tue Sep 13 15:59:44 2016
\usepackage{graphicx}
\usepackage{url}
\usepackage{tikz}
...
\begin{abstract}
  ...
  {\em i.e. black holes}, of less than about \DIFdelbegin \DIFdel{one quarter, $-0.53<\eta_N<0.26$
  from }\DIFdelend \DIFaddbegin \DIFadd{two-thirds, $\eta_N<0.68$
  from the gravitational interaction of }\DIFaddend M\,\DIFdelbegin \DIFdel{32 from using the best determined distances to }\DIFdelend \DIFaddbegin \DIFadd{87 with its neighbours in
  the Virgo cluster.  Although }\DIFaddend M\,\DIFdelbegin \DIFdel{31 and
  M}%DIFDELCMD < \,%%%
\DIFdel{32.  If M}%DIFDELCMD < \,%%%
\DIFdel{31 and M}%DIFDELCMD < \,%%%
\DIFdelend 32 \DIFdelbegin \DIFdel{have the maximal relative separation
  allowed within the uncertainties of \mbox{%DIFAUXCMD
\citet{2006AJ....131.1405K}
}%DIFAUXCMD
, the limit is much weaker with $\eta_N<0.99$.  The constraintsfrom
  }\DIFdelend \DIFaddbegin \DIFadd{appears to be a good candidate
  ...
  ellipticals of }\DIFaddend M\,\DIFdelbegin \DIFdel{87 are substantially weaker but could }\DIFdelend \DIFaddbegin \DIFadd{31, substantially better constraints could be
  obtained. In all cases the constraints could }\DIFaddend improve dramatically
...
galaxies and the location of the bottom of the \DIFdelbegin \DIFdel{graviational }\DIFdelend \DIFaddbegin \DIFadd{gravitational }\DIFaddend potential
...
\begin{equation}
  ...
  \DIFdelbegin %DIFDELCMD < \label{eq:14}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{eq:3}
\DIFaddend \end{equation}
In the context of the \DIFdelbegin \DIFdel{parameterized }\DIFdelend \DIFaddbegin \DIFadd{parametrized }\DIFaddend post-Newtonian treatment of
deviations from general relativity \citep{Will:lrr},
\begin{equation}
  \eta_{N} =4\DIFdelbegin \DIFdel{\betaâ}\DIFdelend \DIFaddbegin \DIFadd{\beta - }\DIFaddend \gamma \DIFdelbegin \DIFdel{âˆ’ 3â}\DIFdelend \DIFaddbegin \DIFadd{- 3 - }\DIFaddend \frac{10}{3} \xi - \alpha_1 +
  \frac{2}{3} \alpha_2 - \frac{2}{3} \zeta_1 - \frac{1}{3} \zeta_2.
  \DIFaddbegin \label{eq:4}
\DIFaddend \end{equation}
...
\DIFdelbegin \DIFdel{asymetric }\DIFdelend \DIFaddbegin \DIFadd{asymmetric }\DIFaddend jets the black hole should lie at the bottom of the
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{eq:5}
\DIFaddend \end{equation}
...
\begin{equation}
  ...
  \DIFdelbegin %DIFDELCMD < \label{eq:4}
%DIFDELCMD < %%%
...
by violation of the equivalence principle, Eq.~\DIFdelbegin \DIFdel{\ref{eq:3}}\DIFdelend \DIFaddbegin \DIFadd{\ref{eq:5}}\DIFaddend , then we
...
density, Eq.~\DIFdelbegin \DIFdel{\ref{eq:4}}\DIFdelend \DIFaddbegin \DIFadd{\ref{eq:6}}\DIFaddend .  On the other hand, we will assume that the
...
\begin{equation}
  ...
  \DIFdelbegin %DIFDELCMD < \label{eq:5}
%DIFDELCMD < %%%
...
If one directly used Eq.~(\DIFdelbegin \DIFdel{\ref{eq:4}}\DIFdelend \DIFaddbegin \DIFadd{\ref{eq:6}}\DIFaddend ) to calculate the \DIFdelbegin \DIFdel{restroring
}\DIFdelend \DIFaddbegin \DIFadd{restoring }\DIFaddend force
...
it.  We will compare \DIFdelbegin \DIFdel{this estimate }\DIFdelend \DIFaddbegin \DIFadd{the estimate including the effect of neighbouring
stars (Eq.~\ref{eq:7}) }\DIFaddend with the detailed measurements of the potential
near the core of the elliptical galaxy, M\,87, \DIFaddbegin \DIFadd{as inferred from
stellar velocities }\DIFaddend and find good agreement.
...
\frac{\Delta}{1-\Delta} \frac{M}{d^3} \frac{a^3}{m},
  \DIFdelbegin %DIFDELCMD < \label{eq:13}
%DIFDELCMD < %%%
...
\begin{equation}
  ...
  \DIFdelbegin %DIFDELCMD < \label{eq:7}%%%
\DIFdelend \DIFaddbegin \label{eq:9}\DIFaddend .
\end{equation}
For the galaxy M\,32 \DIFdelbegin \DIFdel{we }\DIFdelend \DIFaddbegin \DIFadd{as a first approximation we will use a Hernquist
...
would entrain the \DIFdelbegin \DIFdel{neighboring }\DIFdelend \DIFaddbegin \DIFadd{neighbouring }\DIFaddend stars, so the stellar nucleus should be
\DIFdelbegin \DIFdel{centered }\DIFdelend \DIFaddbegin \DIFadd{centred }\DIFaddend on the black hole. To measure the possible deviation of the
...
on the Hubble Space \DIFdelbegin \DIFdel{Telscope }\DIFdelend \DIFaddbegin \DIFadd{Telescope }\DIFaddend (HST) in the filter, F555W.  This
...
0.01~arcseconds as do the ellipses and circles \DIFdelbegin \DIFdel{centered }\DIFdelend \DIFaddbegin \DIFadd{centred }\DIFaddend on the stellar
...
\DIFaddbegin \DIFadd{Before we focus on the smaller elliptical galaxies of the M}\,\DIFadd{31
system, we will examine the central region of M}\,\DIFadd{32 in further detail.
\mbox{%DIFAUXCMD
\citet{1998AJ....116.2263L}
}%DIFAUXCMD
deconvolved images with HST and WFPC-1 and
...
M}\,\DIFadd{32 from ground-based data \mbox{%DIFAUXCMD
\citep{1991A&A...243L..17M}
}%DIFAUXCMD
and HST WFPC2
images \mbox{%DIFAUXCMD
\citep{1998AJ....116.2263L}
}%DIFAUXCMD
as well as the WFC3 images and the
...
model.
  ...
  \caption{\DIFaddFL{The surface brightness in the core of M}\,\DIFaddFL{32 from
    \mbox{%DIFAUXCMD
\citet{1998AJ....116.2263L}
}%DIFAUXCMD
, \mbox{%DIFAUXCMD
\citet{1991A&A...243L..17M}
}%DIFAUXCMD
and the photometry and models used in this paper.  We assumed
    $B-F555W=1$ to place the \mbox{%DIFAUXCMD
\citet{1991A&A...243L..17M}
}%DIFAUXCMD
data
    on this plot.}}
...
\DIFadd{If we examine Eq.~\ref{eq:7}, we see that the spring constant of the
Hernquist model that best reflects the central region of M}\,\DIFadd{32 is
...
\DIFadd{The current astrometry yields $\Delta \theta<10$~milliarcseconds,
yielding a weaker constraint of $\Delta<0.96$.  
}

\DIFaddend The three smaller elliptical galaxies, NGC\,205, NGC\,147 and NGC\,185,
...
\DIFdelbegin \DIFdel{centered }\DIFdelend \DIFaddbegin \DIFadd{centred }\DIFaddend on M\,84 and M\,86 about $1.3^\circ$ away.  M\,89 is also a
...
\begin{equation}
  ...
  \mathrm{arcseconds}.
  \DIFdelbegin %DIFDELCMD < \label{eq:16}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{eq:13}
\DIFaddend \end{equation}
...
about 7~arcseconds.  \DIFaddbegin \DIFadd{Furthermore, in the core of M}\,\DIFadd{87 we have
...
\begin{equation}
  ...
  \DIFdelbegin %DIFDELCMD < \label{eq:18}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{eq:14}
\DIFaddend \end{equation}
and
\begin{equation}
    \Delta \theta = 0.014 \frac{\Delta}{1-\Delta} \cos^3 i~
    \mathrm{arcseconds}
    \DIFdelbegin %DIFDELCMD < \label{eq:17}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{eq:15}
\DIFaddend \end{equation}
...
in the optical.  The innermost isophotes will be \DIFdelbegin \DIFdel{centered }\DIFdelend \DIFaddbegin \DIFadd{centred }\DIFaddend on the black
...
\DIFdelbegin \DIFdel{Telscope }\DIFdelend \DIFaddbegin \DIFadd{Telescope }\DIFaddend (HST) in the ultraviolet filter, \DIFdelbegin \DIFdel{F225W}\DIFdelend \DIFaddbegin \DIFadd{F275W}\DIFaddend .  We used images from
...
\citet{2010ApJ...717L...6B}.  They explicitly masked the \DIFdelbegin \DIFdel{noticable }\DIFdelend \DIFaddbegin \DIFadd{noticeable }\DIFaddend jet
...
measurements that we use are in \DIFdelbegin \DIFdel{F225W }\DIFdelend \DIFaddbegin \DIFadd{F275W }\DIFaddend with a higher angular resolution
...
Eq.~(\DIFdelbegin \DIFdel{\ref{eq:17}}\DIFdelend \DIFaddbegin \DIFadd{\ref{eq:15}}\DIFaddend ) and a favourable geometry of $i\approx 0^\circ$.
...
\begin{figure}
  ...
  \caption{Central region of M\,87 \DIFdelbeginFL \DIFdelFL{focussing }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{focusing }\DIFaddendFL on the AGN.  Several
...
several reasons.\footnote{Note also the environmental dependence of
  $\Delta$ is often such that $\Delta$ on the left hand side of Eq.~(\DIFdelbegin \DIFdel{\ref{eq:3}}\DIFdelend \DIFaddbegin \DIFadd{\ref{eq:5}}\DIFaddend )
...
considered
%%%LH 
just the force on the black hole and the \DIFdelbegin \DIFdel{cotery
}\DIFdelend \DIFaddbegin \DIFadd{coterie
}\DIFaddend of stars in its immediate gravitational influence.  Moving the black
...
\DIFdelbegin \DIFdel{$-0.53<\eta_N<0.26$, competitive with the results from pulsar timing
\mbox{%DIFAUXCMD
\citep{Stairs:2005}
}%DIFAUXCMD
.
...
%DIFDELCMD < %%%
\DIFdel{Furthermore, the }\DIFdelend \DIFaddbegin \DIFadd{$\eta_N<0.96$, using a density model adapted to fit the central
regions of M}\,\DIFadd{32.  The }\DIFaddend discovery of a supermassive black hole in one
...
the constraints comes from the unknown geometry of the system \DIFdelbegin \DIFdel{as we
saw }\DIFdelend \DIFaddbegin \DIFadd{and the
central potential as we demonstrate }\DIFaddend for M\,32 in particular so further
information on the relative distances \DIFaddbegin \DIFadd{and the central kinematics }\DIFaddend of
the various galaxies would yield better constraints on the results.
\DIFaddbegin \DIFadd{Observations of the black hole in M}\,\DIFadd{87, the stellar kinematics near
...
.  }\DIFaddend Perhaps other interacting and nearly interacting
...
\DIFaddbegin {\noindent \bf \DIFadd{Acknowledgements}}

\DIFaddend This research is based on NASA/ESA Hubble Space Telescope observations
...
Legacy Archive \DIFaddbegin \DIFadd{which is a collaboration between the Space Telescope
...
Centre (CADC/NRC/CSA)}\DIFaddend . These observations are associated with proposal
...
Study for hospitality\DIFaddbegin \DIFadd{.  The authors would also like to thank the
anonymous referee for detailed comments that improved the paper
substantially}\DIFaddend .
